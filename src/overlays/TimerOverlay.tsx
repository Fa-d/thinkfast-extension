...
export const TimerOverlay: React.FC<Props> = ({ session, context, onResult }) => {
  ...
  const loadContent = async () => {
    const selectedContent = InterventionEngine.selectContent('TIMER', context);
    setContent(selectedContent);

    // Track intervention shown
    AnalyticsService.trackInterventionShown('TIMER', selectedContent.type, context);
  };

  const loadFriction = async () => {
    const friction = await FrictionManager.getFrictionConfig();
    setCountdown(friction.delaySeconds);

    if (friction.delaySeconds === 0) {
      setCanProceed(true);
    } else {
      // Start countdown
  ...
  if (!content) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900 dark:to-blue-900 z-[999999] flex items-center justify-center">
    ...
    >
      <div className="w-full max-w-2xl flex flex-col justify-between" style={{ minHeight: '80vh' }}>
        ...
        <div className="text-center">
          <h1 className="text-2xl font-medium mb-2" style={{ color: 'var(--intervention-text-primary)' }}>
            {session.siteName}
          </h1>
          <p className="text-base" style={{ color: 'var(--intervention-text-secondary)' }}>
            {Math.floor(session.totalDuration / 60000)}-Minute Alert
          </p>
        </div>
        ...
        <div className="w-full flex flex-col items-center">
          ...
          {!canProceed ? (
            <div className="text-center space-y-4">
              <p className="text-base" style={{ color: 'var(--intervention-text-primary)', opacity: 0.9 }}>Take a moment to consider...</p>
            ...
            <div className="w-full space-y-4">
              {/* Go Back Button - Full width, 56px height */}
              <button
                onClick={handleGoBack}
                className="w-full h-14 flex items-center justify-center rounded-xl font-semibold text-lg transition-all active:scale-95 shadow-lg"
                style={{
              ...
              {/* Proceed Button - Full width, 48px height, outlined */}
              <button
                onClick={handleProceed}
                className="w-full h-12 flex items-center justify-center rounded-xl font-medium text-base transition-all hover:bg-white/10 active:scale-95"
                style={{
